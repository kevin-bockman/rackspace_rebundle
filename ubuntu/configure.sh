...
shadowconfig on
cp /root/files/syslog-ng.conf /etc/syslog-ng/
...
#
# Image 112: Replace agent-smith with nova-agent
# 
...
# 10.04+ Use tty1 for console
sed -i "s/hvc0/tty1/g" /etc/init/hvc0.conf

# MOTD
rm -f /etc/update-motd.d/10-help-text || true
[ -f /etc/update-motd.d/99-footer ] && mv /etc/update-motd.d/99-footer /etc/update-motd.d/10-rightscale-message

# NTP (w-3981)
echo "xen.independent_wallclock=1" > /etc/sysctl.d/60-rightscale-ntp.conf
sed -i "s/server ntp.ubuntu.com/server ec2-us-east.time.rightscale.com burst iburst\nserver ec2-us-west.time.rightscale.com burst iburst\nserver time.rightscale.com burst iburst/" /etc/ntp.conf

./configure-common.sh $rightlink_ver
...
echo "You will need to run the cleanup.sh script to sanitize installation."
